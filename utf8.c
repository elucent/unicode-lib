...
static int inited = 0;
    ...
    static void init_lookup_table() {
        for (int i = 1; i < sizeof(specific_categories) / sizeof(range*); i ++) {
...
unsigned long int utf8_length(const char* str, unsigned long int str_length) {
    ...
    while (i < str_length) {
        if (str[i] < UTF8_TWO) ++ n, ++ i;
        else if (str[i] < UTF8_THREE) ++ n, i += 2;
        else if (str[i] < UTF8_FOUR) ++ n, i += 3;
        else ++ n, i += 4;
    }
